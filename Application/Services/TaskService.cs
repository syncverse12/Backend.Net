...
namespace Graduation_Project.Application.Services
{
    public class TaskService : ITaskService
    {
        ...
        public async Task<Result<PagedResult<TaskResponseDto>>> GetMyTasksAsync(string employeeId, TaskQuery query)
        {
         var tasksQuery = _unitOfWork.Repository<TaskItem>()
            .Query()
            .Include(t => t.Category)
            .Where(t => t.AssignedToUserId == employeeId && !t.IsDeleted);
        ...
        public async Task<Result<bool>> StartTaskAsync(string taskId, string employeeId)
        {
            var task = await _unitOfWork.Repository<TaskItem>().GetByIdAsync(taskId);
        ...
        public async Task<Result<bool>> SubmitTaskAsync(string taskId, string employeeId)
        {
            var task = await _unitOfWork.Repository<TaskItem>().GetByIdAsync(taskId);
